...
// MOBILE (aditivo): reaproveita window.RecompilerConfig.mobileMode (definido em
// recompiler.js) quando presente, lido em tempo de execução.
function isMobileMode() {
    return (typeof window !== 'undefined') && (typeof window.RecompilerConfig !== 'undefined') && !!window.RecompilerConfig.mobileMode;
}

// ============================================================================
// [MODERNO + SPEED PRESET] Configuração runtime — otimizada para velocidade
// ============================================================================
var RenderConfig = {
    // Aspecto de saída em telas modernas
    //   0 = PILLARBOX   — 4:3 fiel com barras laterais (retro-correto)
    //   1 = FIT_WIDTH   — preenche a largura, corta topo/base (imersivo)
    //   2 = STRETCH     — estica 4:3 pra tela toda (SPEED PRESET: padrão)
    aspectMode: 2,

    // Resolução dinâmica — SPEED PRESET: agressivo, reage rápido
    dynamicResolution: true,
    drsTargetMs: 16.67,      // 60 Hz
    drsDownshiftMs: 15.0,    // SPEED: dispara antes (era 19.0)
    drsUpshiftMs: 11.0,      // SPEED: sobe só com folga real (era 13.0)
    drsMinScale: 0.35,       // SPEED: deixa cair até 35% (era 0.55)
    drsMaxScale: 1.0,
    drsStepDown: 0.10,       // SPEED: desce de uma vez (era 0.06)
    drsStepUp: 0.01,         // SPEED: sobe devagar (era 0.02)
    drsWindow: 30,           // SPEED: média curta, reação rápida (era 45)

    // Qualidade adaptativa — SPEED PRESET: off (já estaremos em nearest)
    adaptiveQuality: false,

    // Estado inicial de filtro/dither — SPEED PRESET: nearest + 4x4 nativo
    filterMode: 0,           // 0=nearest, 1=bilinear, 2=xBR
    ditherMode: 0,           // 0=PS1 4x4, 1=8x8, 2=blue noise

    // Mobile UX
    keepScreenOn: true,
    autoFullscreen: false,

    // [WARMUP] Preload automático no construtor
    autoPreload: false,
...
// ============================================================================
// [MODERNO] FramePacer — mede frame time e detecta refresh rate do display
// ============================================================================
function FramePacer(windowSize) {
    this.samples = new Float32Array(windowSize || 90);
    this.idx = 0;
    this.count = 0;
    this.avgMs = 16.67;
    this.p95Ms = 16.67;
    ...
    this.hz = 60;
    this._detectSamples = [];
    this._detectLocked = false;
}

FramePacer.prototype.tick = function (ts) {
    if (this.lastTs > 0) {
        ...
        if (dt > 5 && dt < 40) {
            ...
            if (!this._detectLocked) {
                this._detectSamples.push(dt);
                if (this._detectSamples.length >= 30) {
                    this._detectSamples.sort(function (a, b) { return a - b; });
                    var med = this._detectSamples[15];
                    var candidates = [60, 90, 120, 144, 165];
                    var best = 60, bestErr = 1e9;
                    for (var i = 0; i < candidates.length; i++) {
                        var err = Math.abs(1000 / candidates[i] - med);
                        if (err < bestErr) { bestErr = err; best = candidates[i]; }
                    }
                    this.hz = best;
                    this._detectLocked = true;
                }
            }
        }
    }
...
FramePacer.prototype.compute = function () {
    if (this.count === 0) return;
    var sum = 0;
    for (var i = 0; i < this.count; i++) sum += this.samples[i];
    this.avgMs = sum / this.count;

    var arr = Array.prototype.slice.call(this.samples.subarray(0, this.count));
    arr.sort(function (a, b) { return a - b; });
    this.p95Ms = arr[Math.floor(arr.length * 0.95)];
};

FramePacer.prototype.reset = function () {
    this.count = 0;
    this.idx = 0;
    this.lastTs = 0;
};
...
// ============================================================================
// [MODERNO] AdaptiveQualityManager — DRS + degradação de filtro
// ============================================================================
function AdaptiveQuality() {
    this.scale = 1.0;
    this.lastAdjustFrame = 0;
    this.cooldownFrames = 20;
    this.consecutiveLow = 0;
    this.consecutiveHigh = 0;
    this.degradedFilter = false;
    this.originalFilter = 0;
}

AdaptiveQuality.prototype.update = function (pacer, frameIdx, renderer) {
    if (!RenderConfig.dynamicResolution) return;
    if (frameIdx - this.lastAdjustFrame < this.cooldownFrames) return;
    if (pacer.count < 20) return;

    var avg = pacer.avgMs;

    if (avg > RenderConfig.drsDownshiftMs) {
        this.consecutiveHigh = 0;
        this.consecutiveLow++;
        if (this.consecutiveLow >= 3 && this.scale > RenderConfig.drsMinScale) {
            this.scale = Math.max(RenderConfig.drsMinScale, this.scale - RenderConfig.drsStepDown);
            this.lastAdjustFrame = frameIdx;
            this.consecutiveLow = 0;
            renderer._drsDirty = true;
        }
    }
    else if (avg < RenderConfig.drsUpshiftMs) {
        this.consecutiveLow = 0;
        this.consecutiveHigh++;
        if (this.consecutiveHigh >= 8 && this.scale < RenderConfig.drsMaxScale) {
            this.scale = Math.min(RenderConfig.drsMaxScale, this.scale + RenderConfig.drsStepUp);
            this.lastAdjustFrame = frameIdx;
            this.consecutiveHigh = 0;
            renderer._drsDirty = true;
        }
    ...
    if (RenderConfig.adaptiveQuality && this.scale <= RenderConfig.drsMinScale + 0.001) {
        if (avg > RenderConfig.drsDownshiftMs + 3 && !this.degradedFilter) {
            this.originalFilter = RenderConfig.filterMode;
            if (RenderConfig.filterMode > 0) {
                renderer.setFilterMode(RenderConfig.filterMode - 1);
                this.degradedFilter = true;
            }
        }
    }
};
...
// ============================================================================
// [MODERNO] Wake Lock — mantém a tela ligada durante o jogo (mobile)
// ============================================================================
function WakeLockManager() {
    this.lock = null;
    this.enabled = false;
}
WakeLockManager.prototype.request = function () {
    var self = this;
    ...
    navigator.wakeLock.request('screen').then(function (l) {
        self.lock = l;
        self.enabled = true;
        l.addEventListener('release', function () { self.lock = null; self.enabled = false; });
    }).catch(function () {});
};
WakeLockManager.prototype.release = function () {
...
Uint32Array.prototype.getNumberOfVertices = function () { return this.index / 24; }
Uint32Array.prototype.canHold = function (cnt) { return this.index + (24 * cnt) < (this.length * 4); }
Uint32Array.prototype.reset = function () { this.index = 0; }
Uint32Array.prototype.view = function () { return new Uint32Array(this.buffer, 0, this.index >> 2); }
...
vec4 filterXBR(vec2 coord) {
    vec2 texelSize = vec2(1.0 / 1024.0, 1.0 / 512.0);
    vec2 fp = fract(coord / texelSize);
    vec3 A = texture(uVRAM, coord + vec2(0, -texelSize.y)).rgb;
    vec3 B = texture(uVRAM, coord + vec2(texelSize.x, -texelSize.y)).rgb;
    vec3 C = texture(uVRAM, coord + vec2(texelSize.x, 0)).rgb;
    vec3 D = texture(uVRAM, coord + vec2(texelSize.x, texelSize.y)).rgb;
    vec3 E = texture(uVRAM, coord).rgb;
    vec3 F = texture(uVRAM, coord + vec2(0, texelSize.y)).rgb;
    vec3 G = texture(uVRAM, coord + vec2(-texelSize.x, texelSize.y)).rgb;
    vec3 H = texture(uVRAM, coord + vec2(-texelSize.x, 0)).rgb;
    vec3 I = texture(uVRAM, coord + vec2(-texelSize.x, -texelSize.y)).rgb;
    float lA = getLuminance(A);
    float lB = getLuminance(B);
    float lC = getLuminance(C);
    float lD = getLuminance(D);
    float lE = getLuminance(E);
    float lF = getLuminance(F);
    float lG = getLuminance(G);
    float lH = getLuminance(H);
    float lI = getLuminance(I);
    float d1 = abs(lA - lE) + abs(lB - lE) + abs(lC - lE);
    float d2 = abs(lG - lE) + abs(lH - lE) + abs(lI - lE);
    vec3 color = E;
    if (d1 < d2 * 0.8) {
        color = mix(E, (A + B + C) / 3.0, 0.3);
    } else if (d2 < d1 * 0.8) {
        color = mix(E, (G + H + I) / 3.0, 0.3);
    }
...
void main() {
    if (uFilterMode == 0) {
        vec2 snapped = (floor(vTextureCoord * uResolution) + 0.5) / uResolution;
        fragColor = vec4(texture(uVRAM, snapped).rgb, 1.0);
    } else if (uFilterMode == 1) {
        fragColor = vec4(texture(uVRAM, vTextureCoord).rgb, 1.0);
    } else {
        fragColor = filterXBR(vTextureCoord);
    }
}`;
...
void main() {
    ...
    iy = clamp(iy, 0, 511);
    vec3 rgb;
    rgb.r = texelFetch(uVRAM, ivec2(ix0, iy), 0).a;
    rgb.g = texelFetch(uVRAM, ivec2(ix1, iy), 0).a;
    rgb.b = texelFetch(uVRAM, ivec2(ix2, iy), 0).a;
...
const int ditherTable8x8[64] = int[64](
...
ivec3 rgb5(int w) {
    return ivec3(w & 31, (w >> 5) & 31, (w >> 10) & 31);
}
...
int getDitherValue(ivec2 fc) {
    if (uDitherMode == 0) {
        return ditherTable[(fc.y & 3) * 4 + (fc.x & 3)];
    } else if (uDitherMode == 1) {
        return (ditherTable8x8[(fc.y & 7) * 8 + (fc.x & 7)] - 32) / 2;
    } else {
        int hash = (fc.x * 73856093) ^ (fc.y * 19349663);
        return ((hash & 63) - 32) / 4;
    }
}
void main() {
    ivec2 fc = ivec2(gl_FragCoord.xy);
    int dith = getDitherValue(fc);
    float maskOut = (uMaskSet > 0.5) ? 1.0 : 0.0;
    if (vTextureMode == 7) {
        int x = int(floor(tcx * 1024.0));
        int y = int(floor(tcy * 512.0));
        int w = get16(x * 2, y);
        fragColor = vec4(vec3(rgb5(w)) / 31.0, ((w & 0x8000) != 0) ? 1.0 : 0.0);
        return;
    }
    if (vTextureMode == 3) {
        ivec3 c = ivec3(floor(vColor255 + 0.5));
        if (vDither != 0) c += ivec3(dith);
...
function WebGLRenderer(canvas) {
    ...
    this.vertexBuffer = new Uint32Array(256 * 1024 >> 2)
    ...
    this.filterMode = RenderConfig.filterMode;
    this.ditherMode = RenderConfig.ditherMode;

    this.contextLost = false;

    // CLUT cache
    this._vramGen = 0;
    this._clutCache = Object.create(null);
    this._clutCacheGen = -1;

    // TEXTURE1 binding tracker
    this._boundTex1 = null;

    // ========================================================================
    // [MODERNO] Infra de performance adaptativa
    // ========================================================================
    this._frameIdx = 0;
    this._pacer = new FramePacer(RenderConfig.drsWindow);
    ...
    try {
        ...
        console.error("Error: Unable to get WebGL2 context", e);
        return;
    }
    ...
    if (this.gl) {
        ...
        this._boundTex1 = this.tex8vram;
        this.vertexBuffer.reset();
        this.setupProgramDraw();

        this._setupModernMobile(canvas);

        if (RenderConfig.autoPreload) {
            this.preload(null);
        }
    }
    else {
        console.error("Error: Your browser does not appear to support WebGL2.");
    }
}

// ============================================================================
// [WARMUP] preload(onProgress) — aquece a pipeline antes do primeiro frame
// ============================================================================
WebGLRenderer.prototype.preload = function (onProgress) {
    var gl = this.gl;
    if (!gl) return false;
    if (this._preloaded) {
        if (onProgress) try { onProgress(1.0, 'Warmup já concluído'); } catch (e) {}
        return true;
    }

    var self = this;
    function report(p, msg) { if (onProgress) try { onProgress(p, msg); } catch (e) {} }

    report(0.00, 'Warmup: preparando VRAM');
    try {
        var dummy8 = new Uint8Array(2048 * 512);
        gl.bindTexture(gl.TEXTURE_2D, this.tex8vram);
        gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, 2048, 512, gl.ALPHA, gl.UNSIGNED_BYTE, dummy8);
    } catch (e) { console.warn('[preload] dummy tex8vram falhou', e); }

    report(0.10, 'Warmup: alocando framebuffers');
    try {
        var dummyRGBA = new Uint8Array(1024 * 512 * 4);
        gl.bindTexture(gl.TEXTURE_2D, this.tex16draw);
        gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, 1024, 512, gl.RGBA, gl.UNSIGNED_BYTE, dummyRGBA);
        gl.bindTexture(gl.TEXTURE_2D, this.vramP2);
        gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, 1024, 512, gl.RGBA, gl.UNSIGNED_BYTE, dummyRGBA);
    } catch (e) { console.warn('[preload] dummy framebuffer falhou', e); }

    report(0.22, 'Warmup: validando framebuffers');
    try {
    ...
    var parallelExt = gl.getExtension('KHR_parallel_shader_compile');
    var progs = [this.programDraw, this.programDisplay, this.program24bit, this.programTexture];

    function compileStatusAllDone() {
        for (var i = 0; i < progs.length; i++) {
            var p = progs[i];
            if (!p) return true;
            if (parallelExt) {
                if (!gl.getProgramParameter(p, parallelExt.COMPLETION_STATUS_KHR)) return false;
            } else {
    ...
    function runSyntheticDraw() {
        report(0.65, 'Warmup: draw sintético');
        try {
            gl.bindFramebuffer(gl.FRAMEBUFFER, self.buf16draw);
            gl.viewport(0, 0, 1024, 512);
            gl.useProgram(self.programDraw);
            gl.enable(gl.SCISSOR_TEST);
            gl.scissor(0, 0, 4, 4);
            gl.disable(gl.BLEND);

            self.vertexBuffer.reset();
            self.vertexBuffer.addVertex(0, 0, 0);
            self.vertexBuffer.addVertex(1, 0, 0);
            self.vertexBuffer.addVertex(0, 1, 0);
            self.vertexBuffer.index = 24 * 3;

            gl.bindBuffer(gl.ARRAY_BUFFER, self.canvasBuffer);
            if (isMobileMode()) {
                gl.bufferData(gl.ARRAY_BUFFER, self.vertexBuffer, gl.STREAM_DRAW, 0, self.vertexBuffer.index >> 2);
            } else {
                gl.bufferData(gl.ARRAY_BUFFER, self.vertexBuffer.view(), gl.STREAM_DRAW);
            }
            gl.drawArrays(gl.TRIANGLES, 0, 3);

            gl.bindFramebuffer(gl.FRAMEBUFFER, null);
            gl.disable(gl.SCISSOR_TEST);
            gl.viewport(0, 0, self.canvas.width || 8, self.canvas.height || 8);
            ...
            self.setupProgramDraw();
            self._boundTex1 = null;
            self.vertexBuffer.reset();
        } catch (e) { console.warn('[preload] draw sintético falhou', e); }

        report(0.90, 'Warmup: glFinish');
        try { gl.finish(); } catch (e) {}

        self._preloaded = true;
        report(1.0, 'Warmup concluído');
    }

    if (parallelExt) {
        (function poll() {
            if (compileStatusAllDone()) {
                report(0.60, 'Warmup: pipelines prontos');
                runSyntheticDraw();
            } else {
                setTimeout(poll, 8);
            }
...
// ============================================================================
// [WARMUP] preloadVRAMSnapshot(vramSnapshot) — pré-empurra um dump da VRAM
// ============================================================================
WebGLRenderer.prototype.preloadVRAMSnapshot = function (vramSnapshot) {
    if (!vramSnapshot || vramSnapshot.length !== 512 * 1024) {
        console.warn('[preloadVRAMSnapshot] snapshot inválido');
        return false;
    }
    var gl = this.gl;
    if (!gl) return false;

    this.vram.set(vramSnapshot);
    this._vramGen++;

    var view = new Uint8Array(vramSnapshot.buffer, 0, 2048 * 512);
    gl.bindTexture(gl.TEXTURE_2D, this.tex8vram);
    gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, 2048, 512, gl.ALPHA, gl.UNSIGNED_BYTE, view);
...
// [MODERNO] API pública
// ============================================================================
WebGLRenderer.prototype.setAspectMode = function (mode) {
    RenderConfig.aspectMode = mode | 0;
    this._drsDirty = true;
};

WebGLRenderer.prototype.getPerformanceStats = function () {
    return {
        fps: this._pacer.hz && this._pacer.avgMs > 0 ? (1000 / this._pacer.avgMs) : 0,
        avgMs: this._pacer.avgMs,
        p95Ms: this._pacer.p95Ms,
        refreshHz: this._pacer.hz,
        drsScale: this._adaptive.scale,
        canvasW: this.canvas ? this.canvas.width : 0,
        canvasH: this.canvas ? this.canvas.height : 0,
...
WebGLRenderer.prototype.toggleFullscreen = function () {
    var c = this.canvas;
    if (!c) return;
    if (!document.fullscreenElement) {
        var p = c.requestFullscreen ? c.requestFullscreen({ navigationUI: 'hide' }) :
...
WebGLRenderer.prototype.setDebugOverlay = function (on) {
    RenderConfig.debugOverlay = !!on;
    if (on && !this._debugEl) {
        var el = document.createElement('div');
        el.style.cssText = 'position:fixed;top:6px;right:6px;padding:4px 8px;font:11px/1.35 ui-monospace,monospace;' +
            'color:#0f0;background:rgba(0,0,0,0.55);z-index:99999;border-radius:4px;pointer-events:none;white-space:pre;';
        document.body.appendChild(el);
        this._debugEl = el;
    } else if (!on && this._debugEl) {
...
// ============================================================================
// [MODERNO] Setup mobile
// ============================================================================
WebGLRenderer.prototype._setupModernMobile = function (canvas) {
    var self = this;
    this.canvas = canvas;
    ...
    this.updateHDViewport();

    window.addEventListener('resize', function () { self._drsDirty = true; self.updateHDViewport(); }, { passive: true });

    if (window.visualViewport) {
        window.visualViewport.addEventListener('resize', function () { self._drsDirty = true; self.updateHDViewport(); }, { passive: true });
        window.visualViewport.addEventListener('scroll', function () { self._drsDirty = true; self.updateHDViewport(); }, { passive: true });
    }

    window.addEventListener('orientationchange', function () {
        requestAnimationFrame(function () { self._drsDirty = true; self.updateHDViewport(); });
        setTimeout(function () { self._drsDirty = true; self.updateHDViewport(); }, 220);
    }, { passive: true });

    if (typeof ResizeObserver !== 'undefined') {
        this._ro = new ResizeObserver(function () { self._drsDirty = true; self.updateHDViewport(); });
    ...
    canvas.addEventListener('webglcontextlost', function (e) {
        ...
        self._pacer.reset();
        console.warn('[WebGL] Context lost — aguardando restore');
    }, false);

    canvas.addEventListener('webglcontextrestored', function () {
        self.contextLost = false;
        self._drsDirty = true;
        console.warn('[WebGL] Context restored — chame preload() novamente se quiser reaquecer');
    }, false);
    ...
    document.addEventListener('visibilitychange', function () {
        ...
        if (self._isVisible) {
            self._pacer.reset();
            self._adaptive.lastAdjustFrame = self._frameIdx;
            requestAnimationFrame(function () { self._drsDirty = true; self.updateHDViewport(); });
            if (RenderConfig.keepScreenOn) self._wakeLock.request();
        } else {
            self._wakeLock.release();
        }
    }, { passive: true });

    if (RenderConfig.keepScreenOn && !document.hidden) {
        this._wakeLock.request();
    }

    if (RenderConfig.autoFullscreen) {
        var onFirst = function () {
            canvas.removeEventListener('pointerdown', onFirst);
            self.toggleFullscreen();
        };
        canvas.addEventListener('pointerdown', onFirst, { passive: true });
    }

    if (RenderConfig.debugOverlay) this.setDebugOverlay(true);
};

// ============================================================================
// [MODERNO] updateHDViewport — DPR-aware + DRS-aware
// ============================================================================
WebGLRenderer.prototype.updateHDViewport = function () {
    var canvas = this.canvas;
    if (!canvas) return;

    var mobile = isMobileMode();
    var maxDpr = mobile ? 2 : 3;
    var dpr = window.devicePixelRatio || 1;
    if (dpr > maxDpr) dpr = maxDpr;
    if (dpr <= 0) dpr = 1;

    var drs = this._adaptive.scale;
    this._drsScale = drs;
    var effectiveDpr = dpr * drs;
    ...
    var bufW = Math.max(1, Math.floor(cssW * effectiveDpr));
    var bufH = Math.max(1, Math.floor(cssH * effectiveDpr));

    if (canvas.width !== bufW || canvas.height !== bufH) {
        canvas.width = bufW;
        canvas.height = bufH;
        this._lastCanvasW = bufW;
        this._lastCanvasH = bufH;
...
// ============================================================================
// [MODERNO] getLetterboxViewport — 3 modos de aspecto
// ============================================================================
WebGLRenderer.prototype.getLetterboxViewport = function (areaW, areaH) {
    var cw = this.canvas.width;
    var ch = this.canvas.height;

    // SPEED PRESET default = 2 (STRETCH) — caminho mais rápido
    if (RenderConfig.aspectMode === 2) {
        return { x: 0, y: 0, w: cw, h: ch };
    }

    var srcAspect = areaW / areaH;
    var dstAspect = cw / ch;

    if (RenderConfig.aspectMode === 1) {
        var vw = cw;
        var vh = Math.floor(vw / srcAspect);
        var vy = Math.floor((ch - vh) / 2);
        return { x: 0, y: vy, w: vw, h: vh };
    }

    var vw2, vh2, vx2, vy2;
    if (dstAspect > srcAspect) {
        vh2 = ch;
        vw2 = Math.floor(vh2 * srcAspect);
        vx2 = Math.floor((cw - vw2) / 2);
        vy2 = 0;
    } else {
        vw2 = cw;
        vh2 = Math.floor(vw2 / srcAspect);
        vx2 = 0;
        vy2 = Math.floor((ch - vh2) / 2);
    }
    return { x: vx2, y: vy2, w: vw2, h: vh2 };
};

// ============================================================================
// getClutInfo com cache
// ============================================================================
WebGLRenderer.prototype.getClutInfo = function (cl, tm) {
    if (this._clutCacheGen !== this._vramGen) {
        this._clutCache = Object.create(null);
        this._clutCacheGen = this._vramGen;
    }
    var key = ((cl | 0) * 4 + (tm | 0)) | 0;
    var cached = this._clutCache[key];
    ...
    if (tm === 2) {
        ...
        while (--len >= 0) {
            var pixel = vram[offs++];
            if (pixel !== 0) {
                if (pixel <= 0x7fff) info |= 1;
                else info |= 2;
            }
        }
    }
    this._clutCache[key] = info;
    return info;
};
...
WebGLRenderer.prototype.initShaders = function () {
    try {
        ...
        this.programDraw.uDitherMode = gl.getUniformLocation(this.programDraw, "uDitherMode");
        gl.uniform1i(this.programDraw.uDitherMode, this.ditherMode);
        ...
        this.programDisplay.uFilterMode = gl.getUniformLocation(this.programDisplay, "uFilterMode");
        this.programDisplay.uResolution = gl.getUniformLocation(this.programDisplay, "uResolution");
        gl.uniform1i(this.programDisplay.vram, 0);
        gl.uniform1i(this.programDisplay.uFilterMode, this.filterMode);
        gl.uniform2f(this.programDisplay.uResolution, 1024.0, 512.0);
...
WebGLRenderer.prototype.moveImage = function (sx, sy, dx, dy, w, h) {
    ...
    var o = 0;
    var img = gpu.img;
    img.x = dx; img.y = dy; img.w = w; img.h = h;
    img.pixelCount = w * h;
    var copy = img.buffer;
    var vram = this.vram;
    ...
    if (sx + w <= 1024) {
        for (var j = h; j > 0; --j) {
            var oy = ((sy++) % 512) * 1024;
            copy.set(vram.subarray(oy + sx, oy + sx + w), o);
            o += w;
        }
    } else {
        for (var j = h; j > 0; --j) {
...
WebGLRenderer.prototype.storeImage = function (img) {
    ...
    if (img.x + img.w <= 1024) {
        for (var j = 0; j < img.h; ++j) {
            ...
            const start = o;
            o += img.w;
            vram.set(data.subarray(start, o), offsetY + img.x);
        }
    } else {
        for (var j = 0; j < img.h; ++j) {
            const offsetY = ((img.y + j) % 512) * 1024;
...
WebGLRenderer.prototype.storeImageInTexture = function (img) {
    ...
    if ((img.x + img.w) > 1024) {
        ...
        for (let y = 0; y < img.h; ++y) {
            const bo = y * img.w;
            for (let x = 0; x < w1; ++x) buf1[i1++] = img.buffer[bo + x];
            for (let x = 0; x < w2; ++x) buf2[i2++] = img.buffer[bo + x + w1];
        }
...
WebGLRenderer.prototype.flushVertexBuffer = function (clip) {
    ...
    if (isMobileMode()) {
        gl.bufferData(gl.ARRAY_BUFFER, this.vertexBuffer, gl.STREAM_DRAW, 0, this.vertexBuffer.index >> 2);
    } else {
    ...
    gl.activeTexture(gl.TEXTURE1);
    if (this._boundTex1 !== this.tex8vram) {
        gl.bindTexture(gl.TEXTURE_2D, this.tex8vram);
        this._boundTex1 = this.tex8vram;
    }
...
WebGLRenderer.prototype.setBlendMode = function (mode) {
    ...
    switch (mode & 0xf) {
        case 0:
            ...
            gl.blendFuncSeparate(gl.CONSTANT_ALPHA, gl.CONSTANT_ALPHA, gl.ZERO, gl.ONE);
        ...
        case 3:
            ...
            gl.blendFuncSeparate(gl.CONSTANT_ALPHA, gl.ONE, gl.ZERO, gl.ONE);
            gl.uniform1f(this.programDraw.blendAlpha, 1.0);
            break;
        case 4:
        default:
...
WebGLRenderer.prototype.getVertexBuffer = function (cnt, pid) {
    var select = isMobileMode()
        ? (((pid || 0) & 0x02000000) ? ((gpu.status >> 5) & 3) : 4)
        : (((pid || 0) & 0x02000000) ? ((gpu.status >> 5) & 3) : 4) | (gpu.tp << 4);
...
WebGLRenderer.prototype.drawRectangle = function (data, tx, ty, cl) {
    ...
    var tl = tx + 0;
    var tr = tx + w;
    if (gpu.txflip) { tl = tx + 0; tr = tx - w + 1; }
    var tt = ty + 0;
    var tb = ty + h;
    if (gpu.tyflip) { tt = ty + 0; tb = ty - h + 1; }
...
const clrState = { color: 0, c: 0, size: 1024 * 512 };
clr.fill(0);
...
WebGLRenderer.prototype.clearVRAM = function (x, y, w, h, color, clip) {
    ...
    var vram = this.vram;
    if (x + w <= 1024) {
        const row = clr.subarray(0, w);
        for (let j = 0; j < h; ++j) {
            const offsetY = ((y + j) % 512) * 1024;
            vram.set(row, offsetY + x);
        }
    } else {
        for (let j = 0; j < h; ++j) {
            const offsetY = ((y + j) % 512) * 1024;
            for (let i = 0; i < w; ++i) {
                vram[offsetY + ((x + i) % 1024)] = clrState.c;
            }
        }
    }
...
WebGLRenderer.prototype.fillRectangle = function (data) {
    ...
    if (!w || !h) return;
    if ((x + w) > 1024) { console.log('fillRectangle does not support x-wrap', x, w); return; }
    if ((y + h) > 512) { console.log('fillRectangle does not support y-wrap', h, y); return; }
...
WebGLRenderer.prototype.onVBlankBegin = function () {
    ...
    this._pacer.tick(now);
    this._pacer.compute();
    this._adaptive.update(this._pacer, this._frameIdx, this);
    ...
    if (this.displaymode === 2) {
        ...
        this.updateHDViewport();
        var vp = this.getLetterboxViewport(area.w, area.h);
    ...
    if (this.seenRender) ++this.fpsRenderCounter;
    ++this.fpsCounter;
    this.seenRender = false;

    if (this._debugEl) {
        this._debugAccum++;
        if (this._debugAccum >= 6) {
            this._debugAccum = 0;
            var fps = this._pacer.avgMs > 0 ? (1000 / this._pacer.avgMs).toFixed(1) : '--';
            var drs = (this._adaptive.scale * 100).toFixed(0);
            var lost = this.contextLost ? ' LOST' : '';
            var warm = this._preloaded ? '' : ' [sem warmup]';
            this._debugEl.textContent =
                'FPS ' + fps + ' / ' + this._pacer.hz + 'Hz' + lost + warm + '\n' +
                'frame ' + this._pacer.avgMs.toFixed(2) + 'ms (p95 ' + this._pacer.p95Ms.toFixed(1) + ')\n' +
                'DRS ' + drs + '%  ' + this.canvas.width + 'x' + this.canvas.height + '\n' +
                'draws ' + this.fpsRenderCounter + '/' + this.fpsCounter + '  filt ' + this.filterMode;
        }
    }
}
...
// ============================================================================
// Config pública de filtro/dither
// ============================================================================
WebGLRenderer.prototype.setFilterMode = function (mode) {
    this.filterMode = mode;
    RenderConfig.filterMode = mode;
    if (this.gl && this.programDisplay) {
        this.gl.useProgram(this.programDisplay);
        if (this.programDisplay.uFilterMode) {
            this.gl.uniform1i(this.programDisplay.uFilterMode, mode);
        }
    }
};

WebGLRenderer.prototype.setDitherMode = function (mode) {
    this.ditherMode = mode;
    RenderConfig.ditherMode = mode;
    if (this.gl && this.programDraw) {
        this.gl.useProgram(this.programDraw);
        var loc = this.gl.getUniformLocation(this.programDraw, "uDitherMode");
        if (loc) this.gl.uniform1i(loc, mode);
    }
};