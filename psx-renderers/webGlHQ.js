...
"use strict"
// Ajuste para priorizar a compatibilidade e desempenho em mobile.
// 'quality' padrão é 1.0, mas pode ser menor (ex: 0.5) em perfis mobile.
...
// ------------------------------------------------------------------------
// Vertex Shader de Display (Finalização de Tela) - GLSL ES 3.0
// ------------------------------------------------------------------------
const vertexShaderDisplay =
  ...
  "  if (uTs.z > 1.0) {" + // Ajuste opcional para lace
  "    vTextureCoord.y -= mod(vTextureCoord.y, 0.5);" +
  "  }" +
  "}";

// ------------------------------------------------------------------------
// Fragment Shader 16bit Display (Leitura de Draw Area) - GLSL ES 3.0
// ------------------------------------------------------------------------
const fragmentShader16bit =
  ...
  "  return texture(uVRAM, vec2((tx + 0.0) / 1024.0, (ty + 0.0) / 512.0));" +
  "}"+

  "void main(void) {" +
  "  fragColor = vec4(getColor(tx.x, tx.y).rgb, 1.0);" +
  "}";

// ------------------------------------------------------------------------
// Fragment Shader Texture (Load Image from VRAM 8bit/4bit) - GLSL ES 3.0
// ------------------------------------------------------------------------
const fragmentShaderTexture =
  ...
  "out vec4 fragColor;" +
  "void main(void) {" +
  "  vec4 pixel = texture(uVRAM, vTextureCoord);" +
  "  fragColor = vec4(pixel.aaa, 1.0);" +
  "}";

// ------------------------------------------------------------------------
// Fragment Shader 24bit Display (Leitura de Draw Area 24bit) - GLSL ES 3.0
// ------------------------------------------------------------------------
const fragmentShader24bit =
  ...
  "  float r = texture(uVRAM, vec2((tx + 0.0) / 2048.0, ty / 512.0)).a;" +
  "  float g = texture(uVRAM, vec2((tx + 1.0) / 2048.0, ty / 512.0)).a;" +
  "  float b = texture(uVRAM, vec2((tx + 2.0) / 2048.0, ty / 512.0)).a;" +
  ...
  "  fragColor = getColor(x , floor(tx.y));" +
  "}";

// ------------------------------------------------------------------------
// Vertex Shader de Desenho (Primitives) - GLSL ES 3.0
// ------------------------------------------------------------------------
const vertexShaderDraw =
  ...
  "out float tmx;" + // texture window mask x
  "out float tmy;" + // texture window mask y

  "out float tox;" + // texture window offset x
  "out float toy;" + // texture window offset y

  "out float tcx;" + // texture coordinate x
  "out float tcy;" + // texture coordinate y
...
// ------------------------------------------------------------------------
// Fragment Shader de Desenho (Primitives) - GLSL ES 3.0
// ------------------------------------------------------------------------
const fragmentShaderDraw =
  ...
  "in float tmx;" + // texture window mask x
  "in float tmy;" + // texture window mask y

  "in float tox;" + // texture window offset x
  "in float toy;" + // texture window offset yx

  "in float tcx;" + // texture coordinate x
  "in float tcy;" + // texture coordinate y
  ...
  "  float lo = floor(texture(uTex8, vec2(tx + 0.0, ty) / vec2(2048.0, 512.0)).a * 255.0);" +
  "  float hi = floor(texture(uTex8, vec2(tx + 1.0, ty) / vec2(2048.0, 512.0)).a * 255.0);" +
  "  return hi * 256.0 + lo;" +
  "}" +
  ...
  "  }" +
  "  if (vTextureMode == 1.0) {" +
  "    val = texture(uTex8, vec2(tx / 2048.0, ty / 512.0)).a * 255.0;" +
  "    cx = vClut.x + (val / 1024.0); cy = vClut.y;" +
  "    rgba = getColor(cx, cy);" +
  "  }" +
  "  else" +
  "  if (vTextureMode == 0.0) {" +
  "    val = texture(uTex8, vec2(tx / 4096.0, ty / 512.0)).a * 255.0;" +
  ...
  "    fragColor = getColor(tcx, tcy);" +
  "    return;" +
  "  }" +

  "  if (vTextureMode == 3.0) {" +
  "    fragColor = vec4(vColor.rgb, uBlendAlpha);" +
  "    return;" +
  "  }" +
  ...
  "  fragColor = vec4(2.0 * (vColor.rgb * c.rgb), uBlendAlpha);" +
  "}";
...
function WebGLRenderer(canvas) {
  ...
  try {
    // Tenta WebGL 2.0 primeiro para melhor desempenho, com fallback para 1.0.
    const options = {
      alpha: false,
      antialias: false,
    ...
    this.gl = canvas.getContext("webgl2", options) || canvas.getContext("webgl", options) || canvas.getContext("experimental-webgl", options);
  }
  catch (e) {
...
WebGLRenderer.prototype.initShaders = function () {
  try {
    ...
    this.programDisplay.ts = gl.getUniformLocation(this.programDisplay, "uTs"); // Corrigido para uTs no JS para refletir o shader
...
WebGLRenderer.prototype.initTextures = function () {
  ...
  // Usa gl.RGBA para ser compatível com WebGL1 e FFBO (Frame Buffer Object)
...
WebGLRenderer.prototype.setupBuffers = function () {
  ...
  this.programDisplay.vertexPosition = gl.getAttribLocation(this.programDisplay, "aVertexPosition"); // Corrigido para programDisplay
  ...
  // 8/4-bit video ram
  this.programTexture.vertexPosition = gl.getAttribLocation(this.programTexture, "aVertexPosition"); // Corrigido para programTexture
  gl.enableVertexAttribArray(this.programTexture.vertexPosition);

  this.programTexture.vertexTexture = gl.getAttribLocation(this.programTexture, "aVertexTexture"); // Corrigido para programTexture
...
WebGLRenderer.prototype.createTexture = function (mode) {
  ...
  // Garante que o WebGL use interpolação NEAREST para emulação pixel-perfect
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE); // Corrigido para CLAMP_TO_EDGE
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE); // Corrigido para CLAMP_TO_EDGE
...
WebGLRenderer.prototype.setupProgramDraw = function () {
  ...
  // Offset 12: CLUT (4 bytes)
  gl.vertexAttribPointer(this.programDraw.aclut, 2, gl.UNSIGNED_SHORT, false, 24, 12); // Alterado para UNSIGNED_SHORT
  // Offset 16: Texture Page (4 bytes)
  gl.vertexAttribPointer(this.programDraw.texturePage, 2, gl.UNSIGNED_SHORT, false, 24, 16); // Alterado para UNSIGNED_SHORT
  // Offset 20: Texture Window (4 bytes)
...
WebGLRenderer.prototype.onVBlankBegin = function () {
  ...
  gl.vertexAttribPointer(this.programDisplay.vertexPosition, 2, gl.SHORT, true, 8, 0); // Offset 0 para aVertexPosition (xy)
  gl.vertexAttribPointer(this.programDisplay.vertexTexture, 2, gl.SHORT, false, 8, 4); // Offset 4 para aVertexTexture (uv)
