...
// Aviso: Variáveis como map8, map16, psx, dma, mdc, clearCodeCache e PSX_SPEED
// são dependências globais/do ambiente que devem ser fornecidas externamente.
// Em um ambiente ES6 moderno, você as importaria (e.g., import { psx } from './deps.js';).
...
// Inicialização da tabela de escala de vídeo.
for (let i = 0; i < 256; ++i) {
    video.scale[0 + i] = 0;
    video.scale[256 + i] = i;
...
// Exportação para módulo.
// Se você estiver em um ambiente Node.js, usaria: module.exports = mdc;
// Em um ambiente de módulo ES6 (Web/Mobile), a exportação pode ser assim:
// export default Object.seal(mdc);

// Para manter a funcionalidade como era no IIFE (se for um script não-módulo no navegador):
// window.mdc = Object.seal(mdc);
// Como a instrução era apenas "modificar o código", vou manter a definição
// e o selamento do objeto 'mdc' no escopo global/módulo.
// Retornei o 'mdc' para o escopo global implícito, como faria um módulo.
Object.seal(mdc); 
